...
$$
J'(\theta) = \sum w_i J(\theta)
$$

where $J(\theta)$ is usual error term

For time series data, you can use $w_i = \text{Radial basis function(t)}$
...
## Statistics/Economics Perspective

OLS: Ordinary Least Squares is based on Principle of Least Squares
...
3. Series of $u_i$ are random
   ...
   5. Homoscedascity of variance
     ...
     6. $\sigma^2 (u_i|x_i)$ should be same $\forall i$
...
| ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| The upper and lower bound for estimated __ at a given level of significance | mean                                                         | prediction                                                   |
| SE for Univariate Regression                                 | $s_x \sqrt{ \dfrac{1}{n} + \dfrac{(X^* - \bar X)^2}{\sum (X_i - \bar X)^2}}$ | $s_x \sqrt{ \textcolor{hotpink}{1 +} \dfrac{1}{n} + \dfrac{(X^* - \bar X)^2}{\sum (X - \bar X)^2}}$ |
| SE for Multivariate Regression                               | $s_x \sqrt{\dfrac{1}{n} +x'_{\text{pred}_\text{cent}} (X_\text{cent}'X_\text{cent})^{-1} x_{\text{pred}_\text{cent}}}$ | $s_x \sqrt{\textcolor{hotpink}{1+} \dfrac{1}{n} + x'_{\text{pred}_\text{cent}} (X_\text{cent}'X_\text{cent})^{-1} x_{\text{pred}_\text{cent}}}$ |

$$
x_{\text{pred}_\text{cent}} = x_\text{pred} - \mu_X \\
X_\text{cent} = X - \mu_X
$$
...
Both the intervals are narrowest at $X^* = \bar X$, and gets wider further from this point.
