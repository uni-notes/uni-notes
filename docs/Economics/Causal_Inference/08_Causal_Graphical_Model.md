...
classDef highlight fill:orange;
class z1,z2,z3 highlight
```

| Type       | Should Condition to remove block | Reason                                                                                                                                                                                       |
| ---------- | -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Confounder | ✅                                | $z$ affects $y$                                                                                                                                                                              |
...
Independent var that affects $x$, but does not affect $y$ directly

Variable $iv$ satisfies

1. $iv$ correlated with $x$
2. $iv$ correlated with $y$, right? indirect correlation
3. Every open path connecting $iv$ w/ $y$ has an arrow to $x$
   - $iv$ exogenous to $y$ (desired but not strictly necessary)
   - $iv$ affects $y$ only though correlation with $x$

If we use a model: $\hat y = \beta_0 + \beta_1 x + u$ to estimate $E[y \vert \text{do}(x)]$

| Method    | $E[y \vert \text{do}(x)$]                                                                                                       |
| --------- | ------------------------------------------------------------------------------------------------------------------------------- |
| Indirect  | $\dfrac{\text{Cov}(y, iv)}{\text{Cov}(x, iv)}$                                                                                  |
| Two-Stage | $\dfrac{\beta_1}{\alpha_{1}}$<br>where<br>- $\hat x = \alpha_0 + \alpha_{1} iv$<br>- $\hat y = \beta_0 + \beta_{1} \hat{x}$<br> |
